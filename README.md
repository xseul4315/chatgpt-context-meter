...
主視覺聚焦 Context 負載百分比，並以較小字體顯示「對話正文」與「工具歷史」各自的 Token 與占比。
...
這不是 OpenAI 官方 Context 使用率，而是用來反映長對話負載的實用參考值。工具看不到系統提示、Memory、工具定義、訊息封裝 overhead、後端摘要／壓縮／裁切及當次輸出／推理保留空間。
